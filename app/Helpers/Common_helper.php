...
if (!function_exists('get_phrase')) {
    function get_phrase($phrase = '', $value_replace = array())
    {
        ...
        $settings_lang = get_settings('language');
        \Illuminate\Support\Facades\Log::info('--- get_phrase debug ---');
        \Illuminate\Support\Facades\Log::info('Session language: ' . ($session_lang ?? 'NULL'));
        \Illuminate\Support\Facades\Log::info('Settings language: ' . ($settings_lang ?? 'NULL'));
        // --- DEPURACIÓN FIN ---

        static $language_phrases = null;
        static $active_lan = null;

        if ($active_lan === null) {
            $active_lan = $session_lang ?? $settings_lang;
            \Illuminate\Support\Facades\Log::info('Chosen active_lan: ' . ($active_lan ?? 'NULL'));
        }

        if ($language_phrases === null) {
            $active_lan_id = \App\Models\Language::whereRaw('LOWER(name) = ?', [strtolower($active_lan)])->value('id');
            \Illuminate\Support\Facades\Log::info('Found language ID: ' . ($active_lan_id ?? 'NULL'));
            
            $language_phrases = \App\Models\Language_phrase::where('language_id', $active_lan_id)
                ->pluck('translated', 'phrase')
                ->toArray();
        }

        if (isset($language_phrases[$phrase])) {
            $translated = $language_phrases[$phrase];
        } else {
            $translated  = $phrase;
            try {
                $english_lan_id = \App\Models\Language::where('name', 'English')->value('id');
                if ($english_lan_id) {
                    $phrase_exists = \App\Models\Language_phrase::where('language_id', $english_lan_id)->where('phrase', $phrase)->exists();
                    if (!$phrase_exists) {
                        \App\Models\Language_phrase::create(['language_id' => $english_lan_id, 'phrase' => $phrase, 'translated' => $translated]);
                    }
                }
            } catch (\Exception $e) {
                // In case of DB error during this fallback, just continue
            }
        }
