...
class CourseController extends Controller
{
    ...
    public function course_details(Request $request, $slug)
    {
        ...
        if ($course->exists()) {
            ...
            $page_data['sections']       = Section::with('lessons')->where('course_id', $course_details->id)->orderBy('sort')->get();
            ...
            $page_data['user_enrolled'] = false;
            if (auth()->check()) {
                $page_data['user_enrolled'] = Enrollment::where('course_id', $course_details->id)
