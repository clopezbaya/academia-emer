...
use App\Models\User;
use App\Notifications\NewQuestionInCourse;
...
class ForumController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // If it's a new question (not a reply), notify instructors.
        if ($forum->parent_id == 0) {
            $course = Course::find($forum->course_id);
            if ($course && $course->instructor_ids) {
                $instructor_ids = json_decode($course->instructor_ids, true);
                if(is_array($instructor_ids) && count($instructor_ids) > 0){
                    $instructors = User::whereIn('id', $instructor_ids)->get();
                    // Notification::send($instructors, new NewQuestionInCourse($forum)); // Deshabilitado: Las preguntas se gestionan solo en la plataforma.
                }
            }
        }
