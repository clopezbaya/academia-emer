		...
		if ($host == 'vimeo') {
			...
			$video_id = $match[1];
			                        ...
			                        if (empty($youtube_api_key)) {
			                            ...
			                            return ['error' => 'YouTube API Key no configurada. Por favor, configura tu API Key en los ajustes del sistema.'];
			                        ...
			                        try {
			                            ...
			                            \Log::error('Error al obtener detalles de video de YouTube: ' . $th->getMessage());

			                            return ['error' => 'No se pudieron obtener los detalles del video de YouTube. Verifica la URL o la API Key. Error: ' . $th->getMessage()];

			                        }

			            

			                        if ($hash == '' || empty($hash->items)) return ['error' => 'Video de YouTube no encontrado o inválido.'];

			            

			                        $duration = new DateInterval($hash->items[0]->contentDetails->duration);			return array(
				'provider'          => 'YouTube',
				'video_id'			=> $video_id,
