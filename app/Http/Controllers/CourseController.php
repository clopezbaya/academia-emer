...
class CourseController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Handle preview video link or file
        if ($request->preview_video_provider == 'link') {
            $commonController = new CommonController();
            $videoDetails = $commonController->get_video_details($request, $request->preview_link);

            if (isset($videoDetails['error'])) {
                return ['error' => get_phrase('Error al procesar el enlace de video: ') . $videoDetails['error']];
            }
            if (isset($videoDetails['embed_video'])) {
                $data['preview'] = $videoDetails['embed_video'];
            } else {
                return ['error' => get_phrase('No se pudo obtener la URL de incrustación del video.')];
            }
    ...
    function approval(Request $request, $id)
    {
        ...
        try {
            ...
            \Log::error('Error al enviar correo de aprobación de curso: ' . $e->getMessage());
            Session::flash('error', get_phrase('Course activated, but email notification failed: ') . $e->getMessage());
        }
