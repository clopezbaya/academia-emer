...
class PlayerController extends Controller
{
    public function course_player(Request $request, $slug, $id = '')
    {
        ...
        $lesson = Lesson::where('id', $id)->firstOrNew(); // Fetch lesson details early

        // Handle unauthenticated users
        if (!auth()->check()) {
            if (!$lesson->is_preview) {
                Session::flash('error', get_phrase('Please login or register to view this lesson.'));
        ...
        if ($course->is_paid && auth()->user()->role != 'admin') {
            if (auth()->user()->role == 'student') { // for student check enrollment status
                ...
                if (!$enroll_status && !$lesson->is_preview) {
                    Session::flash('error', get_phrase('Not registered for this course or lesson is not a preview.'));
                    return redirect()->route('course.details', ['slug' => $slug]);
                }
                // If expired, always deny, even if it's a preview (as the course itself is expired)
        ...
        $current_lesson_id = $id; // Use the $id from the request as the current lesson being attempted

        if (auth()->check()) { // Only do this if user is logged in
            ...
            // If no lesson ID was provided in the URL, default to history or first lesson
            if ($current_lesson_id == '') {
                $current_lesson_id = $check_lesson_history->watching_lesson_id ?? $first_lesson_of_course;
            }

            // if user has any watched history or not
            if (!$check_lesson_history && $current_lesson_id > 0) {
                $data = [
                    'course_id'          => $course->id,
                    'student_id'         => auth()->user()->id,
                    'watching_lesson_id' => $current_lesson_id,
            ...
            if ($current_lesson_id > 0) {
                Watch_history::where('course_id', $course->id)
                    ...
                    ->update(['watching_lesson_id' => $current_lesson_id]);
            }
        } else { // Not authenticated, so cannot have watch history or use persisted ID
            if ($current_lesson_id == '') {
                // If not authenticated and no specific lesson ID, redirect to course details
                return redirect()->route('course.details', ['slug' => $slug])->with('error', get_phrase('Please select a lesson to preview.'));
            }
            // If an ID is provided but not authenticated, the authorization above already handled non-preview.
        }

        // Re-fetch based on current_lesson_id (important if $id was empty and defaulted)
        $lesson = Lesson::where('id', $current_lesson_id)->firstOrNew(); 
        if (empty($lesson->id)) { // If no lesson found or ID is invalid after all checks
            Session::flash('error', get_phrase('Lesson not found.'));
            return redirect()->route('course.details', ['slug' => $slug]);
        }

        $page_data['course_details'] = $course;
        $page_data['lesson_details'] = $lesson; // Use the already determined $lesson
        $page_data['history']        = Watch_history::where('course_id', $course->id)->where('student_id', auth()->user()->id)->first(); // History only if authenticated
