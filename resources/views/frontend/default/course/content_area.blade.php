                            ...
                            <div class="accordion-body">
                                <ul class="lesson-list course_list">
                                    @foreach ($section->lessons as $lesson)
                                        <li>
                                            @php
                                                $lesson_link = 'javascript::void(0);';
                                                $is_locked = true;

                                                if ($course_details->is_paid == 0 || $lesson->is_preview || $user_enrolled) {
                                                    $lesson_link = route('course.player', ['slug' => $course_details->slug, 'id' => $lesson->id]);
                                                    $is_locked = false;
                                                }
                                            @endphp
                                            <a href="{{ $lesson_link }}" class="d-flex justify-content-between align-items-center">
                                                <p class="d-flex">
                                                ...
                                                @if($lesson->is_preview)
                                                    <span class="badge bg-success ms-2">{{ get_phrase('Preview') }}</span>
                                                @elseif($is_locked)
