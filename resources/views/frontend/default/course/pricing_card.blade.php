    ...
    <div class="ps-box">
        ...
        @if (isset(auth()->user()->id))
            @if (auth()->user()->role == 'instructor')
                @if (auth()->user()->id == $course_details->user_id) {{-- Es este curso del propio instructor? --}}
                    <a href="{{ route('instructor.course.edit', ['id' => $course_details->id]) }}" class="eBtn gradient w-100 mb-3">
                        <img src="{{ asset('assets/frontend/default/image/enroll.png') }}" alt="..."> {{-- Imagen de placeholder --}}
                        {{ get_phrase('Manage Course') }}
                    </a>
                @else {{-- Curso de otro instructor --}}
                    <a href="javascript::void(0);" class="eBtn learn-btn w-100 mb-3" disabled>
                        {{ get_phrase('Instructor cannot enroll in courses') }}
                    </a>
                @endif
            @else {{-- Logged in, but NOT an instructor (e.g., student or admin) --}}
